...
| P161-163 | Téléchargement patrons payants + récup code | 1 | 🟡 (**backend fait** : tables `patrons`/`patron_achats`, `PaymentService::initiatePatron` via FedaPay **sandbox** + webhook, `PatronController` créateur CRUD (upload privé), endpoints publics acheter/statut/**télécharger par code** (P162-163), patron exposé dans `show()`. Gate plan `patrons_payants` (défaut inclus). Reste : **front** bouton Télécharger + reçu + menu Récupérer + upload créateur) |
