...
| GX-13 | Les artisans étaient rattachés à un **plan mort** | ✅ 23/07 | « On ne peut modifier que les plans designer » : **pas un écran manquant, un bug de données**. L'inscription mettait tout artisan sur un plan **legacy désactivé** (3 500 F) — 3 ateliers y étaient. Modifier « Atelier » en admin ne changeait donc **rien** pour eux, alors que Studio marchait. Corrigé à la source + migration (rebascule **puis** suppression, dans cet ordre à cause de la clé étrangère) + seeder nettoyé. **Vérifié en prod : 11 plans → 5, les 3 artisans basculés, 0 orphelin.** 6 tests. |
| GX-14 | Le texte des plans découle de ce qui est saisi en admin | ✅ 23/07 | Vitrine et offre in-app tenaient **deux listes écrites à la main**, dans deux vocabulaires : un même plan ne se lisait pas pareil, et une clé ajoutée en admin n'apparaissait nulle part. Une **seule dérivation partagée** désormais : 7 sous-ateliers saisis → « Jusqu'à 7 ateliers supplémentaires » des deux côtés. Un quota à 0 fait disparaître la ligne au lieu d'annoncer « 0 client par mois ». Vérifié à l'écran **sur les deux surfaces**. |

| GX-15 | Plans d'abonnement pilotables **de A à Z** depuis l'admin | ✅ 23/07 | « 100 % éditable, c'est pour ça qu'un back-office existe ». **(1) Public** du plan (tous / artisan / designer) → la vitrine et l'offre in-app filtrent dessus ; tout l'existant reste en « tous », **aucun tarif ne bouge**, la direction crée ses plans artisan elle-même. **(2) Visibilité** séparée vitrine / application (`is_actif` mélangeait « existe-t-il ? » et « où le montre-t-on ? »). **(3) Libellés** des lignes éditables et bilingues, avec jeton `{n}` qui prend la valeur du plan — renommer « Photos VIP » demandait avant un développeur **pour un mot**. Durée d'un plan permanent affichée « Permanent » au lieu de « 32000 j ». 11 tests. Vérifié en prod. |
