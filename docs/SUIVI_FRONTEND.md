...
| ID | Sujet | Statut | Détail / où |
|---|---|---|---|
| REL-V1 | Points vitrine ouverts | 🟡 | **Bien plus avancé que marqué — vérifié le 22/07 (Aquilas actif sur cette zone).** P132 (retour depuis inscription/connexion) FAIT, tagué dans le code. Dropdowns Solutions/Tarifs/Documentation STRUCTURELLEMENT présents (`NavDropdown`, VitrineChrome.jsx) — mais avec un contenu différent de la spec initiale sur 3 points, probablement des choix assumés à confirmer plutôt que des oublis : barre de contact = e-mail centré (spec = téléphone/WhatsApp sombre à droite) ; Solutions = créateurs/espace client/artisans (spec = liens app mobile) ; Documentation renvoie vers des pages génériques, pas la page à 3 cartes spécifiques (le HTML « déjà conçu » par la direction n'est pas accessible ici). **Vraiment ouverts** : P138 (tarif au footer) et P139 (CGV complètes), tous deux en attente d'un document de la direction. Détail complet dans `VITRINE_TODO_FRONTEND.md`, remis à jour point par point. |
