...
| ABO-2 | Inscription simplifiée à la volée | ✅ | ✅ **Bien plus fait que marqué — vérifié le 22/07.** Le module s'ouvre bien de lui-même : sur un clic « Suivre » sans compte, `CreateurProfilPage` mémorise l'intention et redirige vers `/espace-client?action=suivre_createur`, où `<Login>` s'affiche DIRECTEMENT (pas caché derrière un clic), avec un message expliquant pourquoi. Seule l'e-mail est demandé (code OTP, pas de mot de passe) + bouton Google en accélérateur. Aucun compte n'est créé tant que le code n'est pas vérifié — abandonner avant validation ne laisse rien. Au retour, l'abonnement se rejoue automatiquement (`reprendreAction`). ⚠️ **Nuance sur « la migration d'un abonnement anonyme »** : la question ne se pose plus. `ABO-1` a déjà rendu le compte OBLIGATOIRE pour s'abonner (le serveur renvoie 401 `auth_requise` avant toute création anonyme) — **0 ligne** en production avec `visitor_key` rempli et `gxt_client_id` vide. Le champ `visitor_key` reste nullable dans `AtelierAbonne` pour d'éventuelles lignes historiques, mais il n'y en a aucune à fusionner aujourd'hui. **Décision prise avec la direction pour si ce cas redevient réel** : fusion automatique, sans écran de confirmation. |
...
| ID | Sujet | Statut | Détail / où |
|---|---|---|---|
| REL-V1 | Points vitrine ouverts | ✅ | **Avancé le 22/07.** P132 (retour inscription/connexion) fait. P180 (barre de contact) et P183-184 (menu Documentation + page dédiée) refaits conformes à la spec exacte fournie. P181/P182 (contenu Solutions/Tarifs) TRANCHÉS : la spec demandait des apps mobiles distinctes par type de compte et des tarifs séparés par type — vérifié dans le code, ni l'un ni l'autre n'existe (compte unique à 2 types, tarif unique) ; le contenu actuel correspond à l'architecture réelle, appliquer la spec littéralement aurait créé des liens creux. **Restent bloqués sur un document manquant de la direction** : P138 (tarif au footer, référence visuelle non fournie), P139 (CGV complètes, nouveau texte annoncé non transmis). **P140 fait après validation explicite du chef** (commit `fe10c4c`) : la demande littérale (fusionner 5 pages légales en une) était dépassée — le hub à barre latérale livré en Pt 121 fait déjà naviguer les 11 pages sans rechargement. Seule la part encore valable a été appliquée : « mentions légales » retirée de la liste (sa route redirigeait déjà vers `/cgu`, l'entrée était un cul-de-sac), CGU remontée en tête, et le composant `MentionsLegalesPage` devenu mort supprimé. **Aucun mot du texte réglementaire n'a été touché.** |
| REL-V2 | Pré-rendu SEO | 🟡 | **Découverte plus large le 22/07 (poursuite de l'audit).** Le backend est en réalité COMPLET, pas juste amorcé : `SeoRenderController` rend l'accueil (vraie liste de créateurs en HTML), la liste `/createurs`, le profil de chaque créateur (données réelles + schema.org), et 13 pages génériques (légales, aide, tarifs…) — configurables sans déploiement via `VitrineSetting::seoPages()`. **Vérifié en direct, en contournant nginx** : l'appel à `GET /api/vitrine/seo/render` fonctionne parfaitement (titre correct, vraie liste de créateurs rendue). Le sitemap (`sitemap/createurs.xml`) est également EN LIGNE et correct. **Le seul chaînon manquant, confirmé par un test en direct sur le site** : envoyer une requête avec un user-agent Googlebot ou Facebook renvoie EXACTEMENT le même titre générique qu'un navigateur normal — nginx ne route encore aucun robot vers ce rendu. Fichier nginx en `600 root:root`, hors de portée du seul accès sudo whitelisté disponible. Configuration prête, transmise à la direction. |
...
| REL-V6 | Kit de formulaire admin (modale + champs) | ✅ | **Fait le 22/07** (`4a949cc` master, `c42f528` android). Parti d'un signalement du chef : le modal des plans était « coupé », impossible d'atteindre le haut du formulaire — un premier élargissement (512 → 896 px, 2 colonnes) n'y avait rien changé. **Vraie cause** : le dialogue n'avait ni hauteur plafonnée ni défilement interne, il comptait sur le scroll du conteneur extérieur combiné à un centrage `items-center` — le début d'un contenu centré plus haut que l'écran devient alors inatteignable. `ui/Modal.jsx` suivait déjà le bon motif mais n'avait jamais servi de référence ici. **Décision du chef** : ne pas rustiner page par page, centraliser (esprit Filament, mais maison — aucune dépendance ajoutée, le sur-mesure est voulu). Créé `components/admin/form/` : `AdminModal` (hauteur plafonnée + corps défilant, tailles jusqu'à `4xl`), `AdminField`, `AdminSelectField`, `AdminNumberField` (bouton ∞ conservé), `AdminToggle`, `AdminFormSection`, `AdminFormGrid`. Les **5 pages** qui codaient leur propre modale et redéfinissaient chacune `INPUT`/`LABEL` l'utilisent (Plans, Offres, Codes promo, Liste noire, Transactions) : **−437 lignes / +303**. Le bug de découpe ne peut plus se reproduire sur une page qui passe par `AdminModal`. **Périmètre strictement admin** — rien touché côté espace professionnel, et **aucun champ, validation ou appel API modifié** : seule la mise en forme bouge. **Puis, feu vert du chef dans la foulée** : les 11 écrans restants migrés aussi (`AdminInfos` — qui avait elle aussi sa propre modale —, `AdminNotifications`, `AdminPaiements`, `AdminParametres`, `AdminReseaux`, `AtelierDetail`, `Banniere`, `Admins`, `PagesLegales`, `ReglagesVitrine`, `Tickets`). Résultat sur les 27 écrans admin : **plus une seule modale codée à la main, ni une seule constante de style recopiée localement**. ⚠️ **Nuance à ne pas gommer** : 5 champs restent volontairement écrits sur place car ce ne sont pas des champs de formulaire ordinaires — la recherche d'`AteliersPage` (`pl-9` pour son icône, fond `bg-subtle`), la zone de réponse de `TicketDetailPage` (`px-4 py-3`, plus aérée), le sélecteur de fréquence d'`AdminDashboardPage` (volontairement petit et `text-dim`), le motif de refus en ligne de liste d'`AdminRealisationsPage`, et `AdminLoginPage` (page autonome, hors `AdminLayout`). Les forcer dans le kit dégraderait l'affichage : ce n'est pas de la dette. La recopie avait engendré **4 variantes de `INPUT` et 3 de `LABEL`** pour le même rôle ; ramenées à 3 habillages qui correspondent à de vrais besoins — `ADMIN_INPUT` (champ de formulaire, pleine largeur), `ADMIN_CONTROL` (largeur fixée par l'appelant : `flex-1`, `sm:w-20`…), `ADMIN_FILTER` (contrôle compact de barre d'outils). Le bricolage `.replace('w-full ', '')` laissé dans `ListeNoirePage` est supprimé au passage, ainsi qu'un import devenu mort (`X` dans `AdminInfosPage`). |
