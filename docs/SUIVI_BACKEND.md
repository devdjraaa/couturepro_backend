...
| REL-8 | Deux APK : identité des saveurs | 🟡 | **Trouvé le 20/07 en construisant l'APK groupé.** Les deux saveurs Android (`gextimo` = pros, `admin` = console interne) portaient le **même `applicationId`** : installer la console REMPLACE l'app des pros au lieu de cohabiter. Trois défauts empilés : (1) `scripts/build-android.sh` remplaçait les **trois** `applicationId` du `build.gradle` avec un `sed` sans ancrage → **corrigé** (ancré sur `defaultConfig`, les saveurs gardent leur identité) ; (2) le script ne restaurait que `capacitor.config.json`, laissant `build.gradle` muté après chaque build → **corrigé** (sauvegarde + restauration) ; (3) **cause racine que le `sed` masquait** : `google-services.json` ne déclare que `com.couturepro.app`, donc avec un identifiant distinct le build admin échoue sur `processAdminDebugGoogleServices`. Les deux saveurs compilent, vérifié. **BLOQUÉ SUR TOI** : enregistrer `com.couturepro.admin` comme application Android dans la **console Firebase**, remplacer `android/app/google-services.json` par le fichier régénéré, puis rétablir l'identifiant distinct (marche à suivre écrite dans `build.gradle`). |
